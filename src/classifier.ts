...
const BASE_SYSTEM_PROMPT = `주어진 메시지만 읽고 개발 작업이 필요한지 아닌지만 판단해라. 1 아니면 0만 출력해라. 다른 텍스트는 절대 출력하지 마라.
...
async function initSession(): Promise<string> {
    ...
    await runCommand("claude", ["-p", "--model", getConfig().classifier.model, "--output-format", "text", "--session-id", classifierSessionId, "--system-prompt", systemPrompt, "준비 완료. 이후 메시지를 분류해라."], getConfig().project.root);
...
export async function classifyMessage(text: string, attachmentNames?: string[]): Promise<ClassifyResult> {
    ...
    try {
        ...
        // --resume으로 기존 세션 이어가기
        const raw = await runCommand("claude", ["-p", "--model", getConfig().classifier.model, "--output-format", "text", "--resume", sessionId, userContent], getConfig().project.root);
