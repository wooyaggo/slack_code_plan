...
function buildPrompt(message: string, attachments: ProcessedAttachment[]): string {
    ...
    for (const attachment of attachments) {
        if (attachment.is_image) {
            sections.push(
                `- 이미지 첨부: ${attachment.original_name}`,
                `  MIME 타입: ${attachment.mimetype}`,
                `  파일 경로: ${attachment.local_path}`,
            );
            continue;
        }

        sections.push(
            `- 텍스트 첨부: ${attachment.original_name}`,
            `  MIME 타입: ${attachment.mimetype}`,
            "[첨부 내용 시작]",
            attachment.content ?? `(내용 없음, 파일 경로: ${attachment.local_path})`,
            "[첨부 내용 끝]",
        );
    }
