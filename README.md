...
```bash
npm i --no-save --no-package-lock github:wooyaggo/slack_code_plan
```
...
```bash
npx slack_code_plan start <CHANNEL_ID> --pm2
```

예시:

```bash
npx slack_code_plan start C0123456789 --pm2
```
...
```bash
npx slack_code_plan start <CHANNEL_ID>
```
